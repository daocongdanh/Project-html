$(document).ready(function () {
  ...
  const loadData = () => {
    ...
    $("#hinhanhminhhoa").attr(
      "src",
      `../../Common/img/${product.thumbnail[0]}`
    );
    $("#idproduct").attr("data-id",idProduct);
    var htmlOptionImage = ``;
    product.thumbnail.forEach((item, index) => {
      htmlOptionImage += `<div class="box-item mb-2">
                            <img style="border-bottom: 2px solid transparent;box-shadow: rgba(149, 157, 165, 0.2) 0px 8px 24px;" src="../../Common/img/${item}" class="opimg ${
        index == 0 ? "selected" : ""
      }" alt="../../Common/img/${item}">
                          </div>`;
    });
  ...
  const loadReviews = (page, sort) => {
    ...
    var reviewsByProduct = reviewList.filter(
    ...
    if (sort == "Oldest") {
      reviewsByProduct.sort(
        (o1, o2) => Date.parse(o1.time) - Date.parse(o2.time)
      );
    }
    if (sort == "Newest") {
      reviewsByProduct.sort(
        (o1, o2) => Date.parse(o2.time) - Date.parse(o1.time)
      );
    }
    // Rate avg
    var tong = 0;
    ...
    var htmlRateAvg = ``;
    for (let i = 1; i <= 5; i++) {
      if (i <= tong) htmlRateAvg += `<i class="fa fa-star "></i>`;
      else htmlRateAvg += `<i class="fa fa-star" style="color: #CCCCCC;"></i>`;
    }
    $("#avgRate1").html(
      htmlRateAvg +
        `<a href="#review"  class="review-count text-black mb-0 ms-3">Reviews (${reviewsByProduct.length})</a>`
    );
    ...
    var totalPage =
      Math.floor(reviewsByProduct.length / 2) +
      (reviewsByProduct.length % 2 == 0 ? 0 : 1);
    ...
    if (
      (page == -1 && pageActive == 1) ||
      (page == -2 && pageActive == totalPage)
    )
      return;
    if (page == -1) {
      // prev
      reviewsByProduct = reviewsByProduct.slice(
        (pageActive - 1 - 1) * 2,
        (pageActive - 1 - 1) * 2 + 2
      );
    ...
    reviewsByProduct.forEach((item) => {
      ...
      htmlReview += `${date[1].length == 1 ? "0" + date[1] : date[1]}-${
    ...
    for (let i = 1; i <= totalPage; i++) {
      ...
      }" data-page="${i}"><span class="page-link">${i}</span></li>`;
    }
    htmlPagination += `
  ...
  const randomProduct = () => {
    ...
    while (1) {
      ...
      if (!productRandom.includes(product)) productRandom.push(product);
      if (productRandom.length == 4) break;
    }
    ...
    productRandom.forEach((item) => {
      htmlRandom += `<div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-12">
                      <div class="item" data-id="${item.id}">
                        <div class="redirect image" href="../../ProductDetail/html/product.html" data-id="${
                          item.id
                        }">
                          <img src="../../Common/img/${
                            item.thumbnail[0]
                          }" class="image-front w-100" alt="">
                          <img src="../../Common/img/${
                            item.thumbnail[1]
                          }" class="image-back w-100" alt="">
                          ${
                            item.sale > 0
                        ...
                        <a class="redirect name d-block" href="../../ProductDetail/html/index.html" data-id="${
                          item.id
                        }">${item.name}</a>
                        <p class="price">
                        `;
  ...
  $(".form-check-size1").click(function (state) {
    ...
    if (!elementSpan.classList.contains("form-check-disable")) {
      ...
      let sizeChoose1 = document.getElementById("sizeChoose1");
      sizeChoose1.innerHTML = valueChoose + " US";

      document.querySelectorAll(".form-check-size1").forEach((e) => {
  ...
  $("#postReview").click(function (e) {
    e.preventDefault();
    document.getElementById("formComment").reportValidity();
